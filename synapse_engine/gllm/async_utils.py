...
class FutureMap:
    ...
    def __init__(
        ...
        self.future_ct = 0
        # GreenBoost local patch (see synapse_engine/NOTICE): was torch.empty
        # (uninitialized GPU memory). resolve_future() can read a slot that
        # store_to_map() hasn't written yet this early in a server's life
        # (this circular buffer only gets fully "warmed up" after enough
        # requests cycle through every slot at least once) — with
        # torch.empty that read returns whatever garbage bit pattern was
        # already sitting in that VRAM region, which as an int64 "token id"
        # is essentially always out of the tokenizer's real vocab range.
        # Confirmed live 2026-07-16: this crashed EVERY request to a freshly
        # started server (both a plain bf16 model and a GPTQ one — proving
        # it's this, not quantization) with `OverflowError: out of range
        # integral type conversion attempted` inside tokenizer.decode,
        # reproducible with or without overlap scheduling. torch.zeros makes
        # an unwritten slot resolve to token id 0 (harmless, always valid)
        # instead of undefined memory content.
        self.token_ids_buf = torch.zeros(
