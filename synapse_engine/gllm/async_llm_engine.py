...
def _log_task_completion(task: asyncio.Task, model_path: str = "") -> None:
    try:
        task.result()
        ...
        # GreenBoost local patch (see synapse_engine/NOTICE): surface engine
        # background-task failures to gb-synapse's dataflux telemetry, not
        # just stderr. Real gap found live 2026-07-16: a crash here (e.g. the
        # incremental-detokenize OverflowError under async/overlap
        # scheduling) previously left the HTTP client hanging with NO signal
        # anywhere the dataflux MCP could see — exactly the "phase that
        # failed emits nothing" blind spot GreenBoost's Observability
        # Must-Rule exists to close. Best-effort: gb_dataflux is only
        # importable when this engine runs under gb-synapse's torch venv
        # (greenboost.pth makes it so); never let telemetry break the
        # engine's own error handling.
        try:
            import gb_dataflux
            gb_dataflux.emit({
                "kind": "synapse_engine_error", "status": "error",
                "model": model_path, "engine": "torch",
                "phase": "background_schedule_task",
...
class PipeAsyncLLM(LLM):
    ...
    def start_schedule_engine(self):
        ...
        self._schedule_task.add_done_callback(
            functools.partial(_log_task_completion, model_path=self.model_path))
