...
#
# Vendored 2026-07-29 from vllm/model_executor/layers/mamba/ops/mamba_ssm.py
# (renamed to avoid confusion with the PyPI `mamba_ssm` package, which this
# repo does not depend on -- everything needed is vendored here as plain
# Triton source, same pattern as the already-vendored causal_conv1d_triton.py
# and ssd_*.py siblings in this directory). Two deliberate deviations from
# upstream, both documented in synapse_engine/NOTICE:
#
# 1. DROPPED: `selective_scan_fn` (the older Mamba-1 scan API) and its CPU/
#    XPU fallback branches. Mamba-2's SSD decode path only needs
#    `selective_state_update`; `selective_scan_fn` pulled in a compiled CUDA
#    extension dependency (`vllm._custom_ops.selective_scan_fwd`) this repo
#    has no reason to build. GreenBoost is GPU-only anyway (Immutable Design
#    Rule, "GPU Compute Always") so the CPU/XPU branches were dead code here
#    regardless.
# 2. SIMPLIFIED: the benchmark-tuned per-GPU JSON config lookup
#    (`get_ssm_configs`/`get_ssm_config_file_name`, keyed on a config
#    directory this repo does not vendor) is replaced by calling the
#    hard-coded `_get_default_ssm_launch_config` heuristic directly --
#    upstream itself falls back to exactly this heuristic whenever no config
#    file is found, which is the *only* outcome possible here since the
#    tuned-config JSON tree was never vendored. This is a documented,
#    deliberate simplification (a real, measurable perf-tuning gap, not a
#    correctness one) -- see synapse_engine/NOTICE.
...
# Sentinel for "no cache slot" in state_batch_indices, matching vLLM's
# vllm/v1/attention/backends/utils.py::NULL_BLOCK_ID (value, not import --
# this repo doesn't vendor vLLM's v1 attention backend).
NULL_BLOCK_ID = 0

TRITON3 = tuple(int(x) for x in triton.__version__.split(".")[:2]) >= (3, 0)
...
def _get_default_ssm_launch_config(
    ...
    """Hard-coded fallback heuristic (BLOCK_SIZE_M, num_warps) for the decode
    kernel below. Upstream (vLLM) prefers a per-GPU benchmark-tuned JSON
    config over this when one is available; this repo doesn't vendor that
    config tree (see the module docstring), so this heuristic is always
...
def try_get_optimal_ssm_config(
    ...
    """Return (BLOCK_SIZE_M, num_warps) for the given kernel shape. See the
    module docstring -- this always resolves to the hard-coded heuristic in
    this vendored copy (no tuned-config JSON tree)."""
    del headdim, batch, nheads, cache_dtype  # unused in the heuristic-only path
