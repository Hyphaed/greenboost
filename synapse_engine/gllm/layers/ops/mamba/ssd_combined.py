...
#
# Vendored 2026-07-29, two changes from upstream: (1) import line adapted
# (no upstream's triton-utils shim in this repo); (2) the trailing
# is_cpu() override block that swaps in a CPU kernel is DROPPED ENTIRELY --
# GreenBoost's Immutable Design Rule forbids CPU tensor compute outright
# (greenboost/CLAUDE.md, "GPU Compute Always"), so the Triton-only
# _mamba_chunk_scan_combined_fwd defined in this file is always used,
# unconditionally. See synapse_engine/NOTICE for the full vendoring note
# (the fixed-heuristic decode-config simplification in mamba2_decode.py is
# the other deliberate deviation from upstream).
