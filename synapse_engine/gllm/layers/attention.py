...
# Log the dense-attention SDPA fallback once per worker (GreenBoost local
# patch, see synapse_engine/NOTICE) — same one-shot-log discipline as the
# MLA decode backend above.
_dense_attn_fallback_logged = False


def _sdpa_paged_varlen_fallback(
    q: torch.Tensor,
    ...
    """GreenBoost local patch (see synapse_engine/NOTICE): torch-SDPA
    fallback for ``_flash_attn_paged_varlen`` on GPUs sgl_kernel's bundled
    flash_attn build has no compiled kernel image for (confirmed live,
    2026-07-16, Blackwell/sm120 RTX 5070 — ``flash_attn_with_kvcache`` has
    no ``is_fa3_supported()`` guard at all, unlike its sibling
    ``flash_attn_varlen_func``, so it crashes the CUDA context instead of
    raising a catchable error).

    Gathers each sequence's KV out of the paged cache into a contiguous
    tensor and runs ``torch.nn.functional.scaled_dot_product_attention``
    per sequence — a Python loop over the batch, not vectorized. This is a
    correctness-first fallback for otherwise-unsupported hardware, not a
    performance path: sgl_kernel's flash_attn stays the fast path wherever
    ``_FA3_AVAILABLE`` is True (Ampere/Hopper).

    SDPA's own ``is_causal`` is top-left-aligned and only correct when
    q_len == kv_len; here q_len can be smaller than kv_len (decode, or a
    prefill continuing a cache from an earlier chunk), so the causal mask
    is built explicitly, bottom-right-aligned to match
    ``flash_attn_with_kvcache``'s own documented convention (query row j
    may attend to key position ``kv_len - q_len + j`` and everything
    before it).
    """
    num_blocks, block_size, num_kv_heads, head_dim = k_cache.shape
    num_heads = q.shape[1]
    batch_size = cache_seqlens.shape[0]
    cu_seqlens_q_cpu = cu_seqlens_q.to("cpu", non_blocking=False)
    cache_seqlens_cpu = cache_seqlens.to("cpu", non_blocking=False)
    ...
    for i in range(batch_size):
        ...
        if q_len == 0:
            continue
        kv_len = int(cache_seqlens_cpu[i])

        q_i = q[q_start:q_end]  # [q_len, num_heads, head_dim]

        n_blocks_needed = (kv_len + block_size - 1) // block_size
        block_ids = page_table[i, :n_blocks_needed].long()
        k_i = k_cache[block_ids].reshape(-1, num_kv_heads, head_dim)[:kv_len]
        v_i = v_cache[block_ids].reshape(-1, num_kv_heads, head_dim)[:kv_len]
        ...
        outputs.append(out_i.squeeze(0).transpose(0, 1))  # [q_len, num_heads, head_dim]
...
def _flash_attn_paged_varlen(
    ...
    """
    # GreenBoost local patch (see synapse_engine/NOTICE): sgl_kernel's
    # flash_attn_with_kvcache has no hardware-capability guard, unlike
    # flash_attn_varlen_func's is_fa3_supported() check — it crashes the
    # CUDA context on GPUs its bundled build has no kernel image for
    # (confirmed: Blackwell/sm120) instead of raising a catchable error.
    # _FA3_AVAILABLE is the same capability check; reuse it here since it's
    # the only signal this build gives us for "does the compiled flash_attn
    # kernel actually support this GPU".
    if not _FA3_AVAILABLE:
        global _dense_attn_fallback_logged
        if not _dense_attn_fallback_logged:
            _dense_attn_fallback_logged = True
            logger.warning(
                "sgl_kernel flash_attn has no kernel image for this GPU "
                "(%s); using the torch SDPA paged-attention fallback for "
                "dense-model attention (slower — see synapse_engine/NOTICE).",
                _FA3_IMPORT_ERROR or "unsupported compute capability",
            )
        if return_softmax_lse:
            raise NotImplementedError(
                "return_softmax_lse is not supported by the SDPA "
                "paged-varlen fallback")
        return _sdpa_paged_varlen_fallback(
            q=q, k_cache=k_cache, v_cache=v_cache, cu_seqlens_q=cu_seqlens_q,
