...
# ---- GPTQ-packed fusion (GreenBoost local patch, see synapse_engine/NOTICE) -
#
# GPTQ's qweight/qzeros/scales are [rows, out_features(_packed)] — the
# OPPOSITE axis convention from the plain weight tensor's
# [out_features, in_features] that copy_qkv_proj/copy_gate_up_proj above
# assume (row-slice). Fusing separate q_proj/k_proj/v_proj (or
# gate_proj/up_proj) GPTQ tensors therefore needs a column-slice variant.
# g_idx ([in_features], not output-sharded) is copied verbatim — identical
# across q/k/v since it depends only on the (shared) quant config, not the
# per-projection output size.

def copy_qkv_proj_gptq_cols(dst, src_q, src_k, src_v):
    """QKV fusion for GPTQ-packed tensors ([rows, out_features(_packed)]).
    Column widths are read directly off each source tensor's own shape
    (already correctly packed on disk — qweight/scales/qzeros all pack
    differently along the output axis) rather than recomputed from
    num_heads/head_dim/pack_factor, so one function serves all three
    tensor kinds unlike the plain-weight row-slice helpers above."""
    tp_size, tp_rank = get_tp_size(), get_tp_rank()
    q_cols = src_q.shape[1] // tp_size
    kv_cols = src_k.shape[1] // tp_size
    dst[:, :q_cols] = src_q[:, tp_rank * q_cols:(tp_rank + 1) * q_cols]
    dst[:, q_cols:q_cols + kv_cols] = src_k[:, tp_rank * kv_cols:(tp_rank + 1) * kv_cols]
    dst[:, q_cols + kv_cols:] = src_v[:, tp_rank * kv_cols:(tp_rank + 1) * kv_cols]


def copy_gate_up_proj_gptq_cols(dst, src_gate, src_up):
    tp_size, tp_rank = get_tp_size(), get_tp_rank()
    cols = src_gate.shape[1] // tp_size
    dst[:, :cols] = src_gate[:, tp_rank * cols:(tp_rank + 1) * cols]
    dst[:, cols:] = src_up[:, tp_rank * cols:(tp_rank + 1) * cols]
