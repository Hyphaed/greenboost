...
from .weight_utils import (
    copy_gate_up_proj,
    copy_gate_up_proj_gptq_cols,
    copy_qkv_a_proj,
    copy_qkv_proj,
    copy_qkv_proj_gptq_cols,
    copy_qkv_proj_gqa,
...
_GPTQ_LEAF_NAMES = (".qweight", ".qzeros", ".scales", ".g_idx")
...
def h_qkv_proj(ctx: LoadContext, k: str, p: torch.Tensor) -> None:
    ...
    """
    w = ctx.weights
    gptq_leaf = _gptq_leaf(k)
    if gptq_leaf == ".g_idx":
        # Identical across q/k/v (depends only on in_features + group_size,
        # not the per-projection output size) — copy q's verbatim.
        p.copy_(get_tensor_from_dict(w, k.replace("qkv_proj", "q_proj")))
        return
    if gptq_leaf is not None:
        copy_qkv_proj_gptq_cols(
            p,
...
def h_gate_up(ctx: LoadContext, k: str, p: torch.Tensor) -> None:
    ...
    gptq_leaf = _gptq_leaf(k)
    if gptq_leaf == ".g_idx":
        p.copy_(get_tensor_from_dict(w, k.replace("gate_up_proj", "gate_proj")))
        return
    if gptq_leaf is not None:
        copy_gate_up_proj_gptq_cols(
            p,
...
def h_proj_dim1(ctx: LoadContext, k: str, p: torch.Tensor) -> None:
    ...
    GreenBoost local patch (see synapse_engine/NOTICE): GPTQ's g_idx is 1-D
    ([in_features]), unlike qweight/qzeros/scales (2-D) — copy_single_proj_dim1
    assumes `dst.shape[1]` exists and crashes on it (IndexError). qweight/
    qzeros/scales need no special handling here: this is a NON-fused single
    projection, so copy_single_proj_dim1's existing full-tensor-at-TP=1
    behavior already copies them correctly regardless of axis convention.
    """
    if _gptq_leaf(k) == ".g_idx":
...
def h_proj_dim0(ctx: LoadContext, k: str, p: torch.Tensor) -> None:
    ...
    slices the output (row) dim. Same g_idx 1-D special-case as h_proj_dim1."""
    if _gptq_leaf(k) == ".g_idx":
        p.copy_(get_tensor_from_dict(ctx.weights, k))
        return
