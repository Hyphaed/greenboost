...
def enable_activation_scaling(batch_size):
    ...
    """
    # GreenBoost local fix (see third_party/gemlite/NOTICE): `return True`
    # unconditionally hit the small-M / non-EVEN_M masked branch of
    # channel_scale_mode==5's Triton codegen (gemm_kernels.py /
    # gemm_splitK_kernels.py), which fails to compile on this GPU/Triton
    # version regardless of dtype (fp8 or int4) - confirmed 2026-07-10 via
    # mispeech/Dasheng-AudioGen-Multilingual (mt5 encoder attention AND the
    # model's own DiT backbone attention both hit the identical
    # CompilationError at batch_size=1). Restoring the batch-gated logic the
    # upstream TODO already describes avoids that broken path for small
    # batches.
